        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <table class="min-w-full mb-2">
                ...
                <tbody class="bg-white">
                    @forelse ($sportDisciplines as $sportDiscipline)
                        <tr class="hover:bg-purple-100">
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap font-bold border-b border-gray-200">
                                @if ($sportDiscipline->logo)
                                    <img src="{{ $sportDiscipline->logo->image }}" class="h-8 inline"/>
                                @endif
                                <span>{{ $sportDiscipline->name }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200">
                                <a href="{{ route('dashboard.sportDisciplines.show', $sportDiscipline->id) }}" class="inline-block text-purple-600 p-2 rounded-full hover:bg-purple-200">
            ...
            {{ $sportDisciplines->links() }}

            <x-button-link href="{{ route('dashboard.sportDisciplines.create') }}" color="purple">
